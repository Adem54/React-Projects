...
class Step3 extends React.Component {
  ...
  handleKeypress = async e => {
    ...
    if (e.code === "Space") {
      ...
      if (this.state.shouldDoFetch) {
        console.log(
          "%cshouldDoFetch true old için şimdi fetch yapacak burda....",
          "background: black; color: red"
        );

        try {
          //Eğer spacebar a basılırsa fetch yap yoksa sadece gender ve region u değiştir dememiz gerekecek..

          const response = await fetch(
            `https://uinames.com/api/?ext&amount=10&region=${this.state.region}&gender=${this.state.gender}&source=uinames.com `
          );
          ...
          console.log(
            `https://uinames.com/api/?ext&amount=10&region=random&gender=${this.state.gender}&source=uinames.com `
          );
          this.setState(
            {
              people: random,
            ...
            () => console.log("After Click: ", this.state.people)
        ...
        this.setState(
          {
            shouldDoFetch: false//Bu setState i bir üstteki setState içinde yapabilirdik ki her setState de render yapılıyor ondan dolayı setState içinde değiştirdiğimiz state leri ayrı ayrı setState açmak yerine bir tane setState açıp onun içinde yapabiliriz....
          },
          () =>
            console.log(
              "shouldDoFetch false mu bakalım... ",
              this.state.shouldDoFetch
            )
        );
      } else {
        console.log(
          "shouldDoFetch false olarak döndüğü için burda fetch yapamayacak.."
        );
      }
  ...
  //Acaba componentdidupdate ile ben render dan sonra handlekeypress teki bir statti değiştirsem ondan sonra handle keypress e her tıklama da fetch yapamaması için bunu nasıl yaparım.... ama şunu untumaki componentdidupdate işlemi her render dan sonra çalışır yapacağın işlemler heryeri etkiler....
  componentDidUpdate(PreviousProp, PreviousState) {
    //console.log("PreviousState: ",PreviousState.people);
    if (
      PreviousState.genderCheck !== this.state.genderCheck ||
      PreviousState.regionCheck !== this.state.regionCheck
    ) {
      console.log("PreviousState.genderCheck: ", PreviousState.genderCheck);
      console.log("this.state.genderCheck: ", this.state.genderCheck);
      console.log("Gender butonlarından birine tıklamışsın.......");
      this.setState({
        shouldDoFetch: true //Sen burayı değiştirir değiştirmez bir daha render  yapıyor...Ayrıca her yaptığın işlemler içerisinde kaç tane setState olayı varsa o kadar render yapıyor....
      });
    } else {
      console.log("PreviousState.genderCheck: ", PreviousState.genderCheck);
      console.log("this.state.genderCheck: ", this.state.genderCheck);
      console.log("Gender butonlarına tıklamadın henüz......");
    }
  }
  componentDidMount = async () => {
    ...
    console.log("İlk index değeri ",this.state.index)
    ...
    */
    ...
    this.setState(
      ...
      () =>
        console.log(
          "shouldDoFetch didmount çalışınca  ",
          this.state.shouldDoFetch
        )
    );
  };
    ...
    this.setState(
      {
        ...
        shouldDoFetch: false,
        regionCheck: !this.state.regionCheck
      },
      () => console.log("Region: ", this.state.gender)
    );
  };
  /*BURAYI İNCELEE.....
    ...
    return (
      <div className="container">
        <div className="row" style={{ margin: "20px" }}>
          ...
          <div className="col-md-2" style={{ marginLeft: "-25px",marginRight:"20px" }}>
            <input
              type="button"
              value="Female"
          ...
          <div className="col-md-2" style={{ marginLeft: "26px" }}>
          ...
          {this.state.error ? (
            ...
            <div key={index} className="col-md-4">
              ...
              >
                ...
                <br></br> {people[index].gender}
                <br></br> {people[index].region}
                <br></br> {people[index].email}
                <br></br> {people[index].birthday.dmy}
              </div>
            </div>
          ) : (
